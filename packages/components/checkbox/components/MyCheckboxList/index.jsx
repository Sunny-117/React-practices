...
export default function MyCheckboxList({
  ...
  const setSingleCheck = useCallback((e, item) => {
    ...
    setCheckedData(checkedData => (
      _checked ? [...checkedData, item] : checkedData.filter(data => data.id !== item.id)
    ));
  })
    ...
    <div>
      <table border='1' width='500' align='center'>
        <thead>
          ...
          {
            headerCells && <tr>
              {headerCells.map(item => (<th key={item}>{item}</th>))}
            </tr>
          }
        </thead>
        <tbody>
                ...
                <tr key={item.id}>
                  <td>
                    <MyCheckbox
                      ...
                      onChange={e => {
                        console.log(item, 'asm')
                        setSingleCheck(e, item)
                      }}
                    >
                    </MyCheckbox>
                  </td>
